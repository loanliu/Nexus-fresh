-- Fix Category Sharing Through Resources
-- Categories are shared through the relationship: Project → Resources → Categories
-- This approach works with the actual database schema

-- 1. Drop the incorrect project_id column if it was added to categories
...
-- 4. Create new RLS policies for categories that work through resources
...
CREATE POLICY "categories_select_policy" ON categories
    ...
    USING (
        ...
        OR id IN (
            ...
            FROM categories c
            JOIN resources r ON r.category_id = c.id
...
SELECT 
    ...
    CASE 
        ...
        WHEN c.id IN (
            ...
            FROM categories c2
            JOIN resources r ON r.category_id = c2.id
            JOIN project_members pm ON pm.project_id = r.project_id
            WHERE pm.user_id = auth.uid()
        ) THEN 'shared_via_resources'
        ELSE 'unknown'
    END as access_type,
    -- Show which projects use this category through resources
    (
