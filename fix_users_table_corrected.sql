-- Fix Users Table and Authentication Setup (CORRECTED VERSION)
...
-- 5. Create RLS policies that allow user creation (FIXED TYPE CASTING)
...
-- 12. Alternative: If you still get errors, try these simpler policies
-- DROP POLICY IF EXISTS "Users can view own profile" ON public.users;
-- DROP POLICY IF EXISTS "Users can update own profile" ON public.users;
-- DROP POLICY IF EXISTS "Enable insert for authenticated users only" ON public.users;
...
-- 13. Test the setup (optional - uncomment to test)
-- SELECT 
--   schemaname,
--   tablename,
--   rowsecurity 
...
-- 14. Show current policies (optional - uncomment to see policies)
-- SELECT 
--   schemaname,
--   tablename,
--   policyname,
